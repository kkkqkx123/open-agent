...
class PromptErrorHandler(BaseErrorHandler):
    ...
    def __init__(self):
        ...
        self._recovery_strategies_old: Dict[Type[PromptError], Callable] = {}
    ...
    def _register_default_handlers(self):
        """注册默认错误处理器（兼容性）"""
        self._error_handlers[PromptLoadError] = self._handle_load_error
        self._error_handlers[PromptInjectionError] = self._handle_injection_error
        self._error_handlers[PromptValidationError] = self._handle_validation_error
        self._error_handlers[PromptCacheError] = self._handle_cache_error
        
        # 注册恢复策略（兼容性）
        self._recovery_strategies_old[PromptLoadError] = self._recover_load_error
        self._recovery_strategies_old[PromptCacheError] = self._recover_cache_error
    ...
    def register_recovery_strategy(
        ...
        """注册恢复策略（兼容性）"""
        self._recovery_strategies_old[error_type] = strategy
    ...
    def handle_error(self, error: PromptError) -> Optional[Exception]:
        ...
        try:
            ...
            # 默认处理：记录并返回原错误
            logger.error(f"提示词错误无法恢复: {error}")
            return error
            
        except Exception as e:
            logger.error(f"错误处理过程中发生异常: {e}")
            return error
    ...
    def attempt_recovery(self, error: PromptError, context: Dict[str, Any]) -> Optional[Any]:
        ...
        # 回退到旧的恢复策略
        error_type = type(error)
        strategy = self._recovery_strategies_old.get(error_type)
...
def handle_prompt_error(error: Exception) -> Optional[Exception]:
    ...
    if isinstance(error, PromptError):
        # 使用统一错误处理框架
        context = create_prompt_error_context()
        registry = ErrorHandlingRegistry()
        registry.handle_error(error, context)
        
        # 尝试恢复
        handler = create_default_error_handler()
        recovery_result = handler.attempt_recovery(error, context)
        if recovery_result:
            return None
