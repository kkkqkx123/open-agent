...
class FallbackManager:
        ...
        **kwargs
        ...
        try:
            ...
            if self.strategy == FallbackStrategy.SEQUENTIAL:
                ...
                return self._parallel_fallback(messages, parameters, primary_error, **kwargs)
            elif self.strategy == FallbackStrategy.RANDOM:
                return self._random_fallback(messages, parameters, primary_error, **kwargs)
            elif self.strategy == FallbackStrategy.PRIORITY:
                return self._priority_fallback(messages, parameters, primary_error, **kwargs)
            else:
    ...
    def _sequential_fallback(
        ...
        **kwargs
        ...
        for i, model in enumerate(self.fallback_models):
            ...
            # 检查触发条件
            if model.conditions and not any(condition(primary_error) for condition in model.conditions):
                continue
        ...
        **kwargs
        ...
        available_models = [
            ...
            if model.enabled and (
                not model.conditions or any(condition(primary_error) for condition in model.conditions)
            )
        ]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(available_models)) as executor:
            # 提交所有任务
            future_to_model = {
                executor.submit(self._try_fallback_model, model.name, messages, parameters, **kwargs): model
                for model in available_models
            }
            
            # 等待第一个成功的结果
            for future in concurrent.futures.as_completed(future_to_model, timeout=self.timeout):
                ...
                try:
                    ...
                    logger.warning(f"并行降级到模型 {model.name} 失败: {fallback_error}")
                    self._update_model_usage(model.name, False)
    ...
    def _random_fallback(
        ...
        **kwargs
        ...
        selected_models = random.choices(available_models, weights=weights, k=self.max_attempts)
