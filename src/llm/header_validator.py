            ...
            if header_lower == 'authorization' and header_value.startswith('Bearer '):
                ...
                if self._is_env_var_reference(token):
                    ...
                    if resolved_token is not None:
                        ...
                        logger.warning(f"无法解析Authorization标头的环境变量引用: {token}")
                else:
                ...
                if self._is_env_var_reference(header_value):
                    ...
                    if resolved_value is not None:
                        ...
                        logger.warning(f"无法解析标头 '{header_name}' 的环境变量引用: {header_value}")
                else:
                    resolved_headers[header_name] = header_value
...
class HeaderProcessor:
    ...
    def process_headers(self, headers: Dict[str, str]) -> Tuple[Dict[str, str], Dict[str, str], bool, List[str]]:
        """
        处理HTTP标头
