...
class IGlobalErrorHandler(ABC):
    ...
    def register_error_handler(
        self, error_class: Type[Exception], handler: Callable[[Exception], str]
    ) -> None:
        """注册自定义错误处理器"""
        pass
...
class GlobalErrorHandler(IGlobalErrorHandler):
    ...
    def handle_error(
        ...
        # 使用默认消息模板
        message_template = self._error_messages.get(
            error_type, self._error_messages[ErrorType.UNKNOWN_ERROR]
        )
    ...
    def register_error_type_mapping(
        self, error_class: Type[Exception], error_type: ErrorType
    ) -> None:
        """注册错误类型映射
    ...
    def get_error_stats(self) -> Dict[str, Any]:
        ...
        error_type_counts = Counter(
            context.error_type.value for context in self._error_history
        )

        # 统计错误类
        error_class_counts = Counter(
            context.error.__class__.__name__ for context in self._error_history
        )
        ...
        return {
            ...
            "recent_errors": [
                context.to_dict() for context in self._error_history[-10:]
            ],
        }
...
def handle_error(
    error_type: ErrorType, error: Exception, context: Optional[Dict[str, Any]] = None
) -> str:
    """处理错误的便捷函数
...
def register_error_handler(
    error_class: Type[Exception], handler: Callable[[Exception], str]
) -> None:
    """注册错误处理器的便捷函数
