...
class WorkflowTemplate:
    ...
    def create_workflow(self, name: str, description: str, **kwargs) -> "BusinessWorkflow":
        ...
        workflow = BusinessWorkflow(
            ...
            steps=[self._customize_step(step, config) for step in self.steps],
            transitions=[self._customize_transition(transition, config) for transition in self.transitions],
            rules=[self._customize_rule(rule, config) for rule in self.rules],
            config=config
        )
