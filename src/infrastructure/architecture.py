...
class ArchitectureChecker:
    ...
    def _define_layer_rules(self) -> Dict[str, LayerRule]:
        ...
        return {
            "domain": LayerRule(
                ...
                forbidden_dependencies={"infrastructure", "application", "presentation"},
            ...
            "application": LayerRule(
                ...
                allowed_dependencies={"domain", "infrastructure"},  # 应用层可以依赖领域层和基础设施层
                forbidden_dependencies={"presentation"},
                path_patterns=["src/application/*"]
            ),
            "presentation": LayerRule(
                layer_name="presentation",
                allowed_dependencies={"domain", "infrastructure", "application"},  # 表现层可以依赖所有其他层级
    ...
    def _resolve_import_path(self, module_name: str, current_file: Path) -> Optional[str]:
        """解析导入路径为文件路径"""
        # 处理相对导入
    ...
    def generate_dependency_graph(self) -> Dict[str, Any]:
        ...
        return {
            "layers": {
                layer_name: {
                    "files": [f for f, l in self.layer_mapping.items() if l == layer_name],
