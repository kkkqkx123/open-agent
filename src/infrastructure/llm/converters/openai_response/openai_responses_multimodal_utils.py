"""OpenAI Responses API多模态工具类

提供OpenAI Responses API特定的多模态内容处理功能。
"""
...
class OpenAIResponsesMultimodalUtils(BaseMultimodalUtils):
    """OpenAI Responses API多模态工具类
    
    提供OpenAI Responses API特定的多模态内容处理功能。
    Responses API使用不同的多模态格式，与Chat Completions API有所区别。
    """
    
    def __init__(self) -> None:
        """初始化OpenAI Responses多模态工具"""
        super().__init__()
    ...
    def process_content_to_provider_format(self, content: Union[str, List[Union[str, Dict[str, Any]]]]) -> List[Dict[str, Any]]:
        """将内容转换为OpenAI Responses API格式"""
        if isinstance(content, str):
            return [self._create_input_text_content(content)]
        elif isinstance(content, list):
            return self._process_content_list_for_responses(content)
        else:
            return [self._create_input_text_content(str(content))]
    
    def extract_text_from_provider_content(self, content: List[Dict[str, Any]]) -> str:
        """从OpenAI Responses API格式内容中提取文本"""
        return self._extract_text_from_responses_content(content)
    
    def validate_provider_content(self, content: List[Dict[str, Any]]) -> List[str]:
        """验证OpenAI Responses API格式内容"""
        return self._validate_responses_content(content)
    
    def _create_input_text_content(self, text: str) -> Dict[str, Any]:
        """创建OpenAI Responses API格式的文本内容"""
        return {
            "type": "input_text",
            "text": text
        }
    
    def _create_input_image_content(self, image_url: str) -> Dict[str, Any]:
        """创建OpenAI Responses API格式的图像内容"""
        return {
            "type": "input_image",
            "image_url": image_url
        }
    
    def _process_content_list_for_responses(self, content_list: List[Union[str, Dict[str, Any]]]) -> List[Dict[str, Any]]:
        """处理内容列表为OpenAI Responses API格式"""
        processed_content = []
        
        for item in content_list:
            if isinstance(item, str):
                processed_content.append(self._create_input_text_content(item))
            elif isinstance(item, dict):
                processed_item = self._process_dict_content_for_responses(item)
                if processed_item:
                    processed_content.append(processed_item)
            else:
                # 其他类型转换为文本
                processed_content.append(self._create_input_text_content(str(item)))
        
        return processed_content
    
    def _process_dict_content_for_responses(self, content_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """处理字典内容为OpenAI Responses API格式"""
        content_type = content_dict.get("type")
        
        if content_type == "text":
            return self._create_input_text_content(content_dict.get("text", ""))
        elif content_type == "input_text":
            # 已经是Responses API格式
            return content_dict
        elif content_type == "image":
            return self._process_image_content_for_responses(content_dict)
        elif content_type == "input_image":
            # 已经是Responses API格式
            return content_dict
        elif content_type == "image_url":
            # 从OpenAI格式转换为Responses格式
            image_url = content_dict.get("image_url", {})
            url = image_url.get("url", "")
            if url:
                return self._create_input_image_content(url)
            else:
                self.logger.warning("image_url内容缺少url字段")
                return None
        else:
            # 未知类型，转换为文本
            return self._create_input_text_content(str(content_dict))
    
    def _process_image_content_for_responses(self, image_item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """处理图像内容为OpenAI Responses API格式"""
        ...
        if source.get("type") == "base64":
            ...
            return self._create_input_image_content(data_url)
    ...
    def _extract_text_from_responses_content(self, content: List[Dict[str, Any]]) -> str:
        """从Responses API格式内容中提取文本"""
        text_parts = []
    ...
    def _validate_responses_content(self, content: List[Dict[str, Any]]) -> List[str]:
        """验证OpenAI Responses API格式内容"""
        errors = []
        ...
        for i, item in enumerate(content):
            item_errors = self._validate_responses_content_item(item, i)
            errors.extend(item_errors)
        
        return errors
    
    def _validate_responses_content_item(self, item: Dict[str, Any], index: int) -> List[str]:
        """验证Responses API格式内容项"""
        errors = []
        ...
        if content_type == "input_text":
            text_errors = self._validate_input_text_item(item, index)
            errors.extend(text_errors)
        elif content_type == "input_image":
            image_errors = self._validate_input_image_item(item, index)
    ...
    def _validate_input_text_item(self, text_item: Dict[str, Any], index: int) -> List[str]:
        """验证input_text内容项"""
        errors = []
    ...
    def _validate_input_image_item(self, image_item: Dict[str, Any], index: int) -> List[str]:
        """验证input_image内容项"""
        errors = []
        
        image_url = image_item.get("image_url")
        if not isinstance(image_url, str):
            errors.append(f"图像内容项 {index} 的image_url字段必须是字符串")
        elif not image_url.strip():
            errors.append(f"图像内容项 {index} 的image_url字段不能为空")
        else:
            # 验证URL格式
            url_errors = self._validate_image_url(image_url, index)
    ...
    def create_image_content_from_file(
        ...
        """从文件路径创建OpenAI Responses API格式的图像内容
        
        Args:
            image_path: 图像文件路径
            
        Returns:
            Dict[str, Any]: Responses API格式的图像内容
        """
        import base64
        import mimetypes
    ...
    def create_image_content_from_url(self, url: str) -> Dict[str, Any]:
        """从URL创建OpenAI Responses API格式的图像内容
    ...
    def create_image_content_from_base64(
        ...
        """从base64数据创建OpenAI Responses API格式的图像内容
    ...
    def optimize_content_for_context(
        ...
        for item in content:
            if isinstance(item, dict):
                ...
                elif item.get("type") == "input_image" and remaining_tokens >= 85:
                    # 只有在有足够token时才包含图像
                    optimized_content.append(item)
                    remaining_tokens -= 85
    ...
    def convert_from_openai_format(self, openai_content: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """从OpenAI Chat Completions格式转换为Responses API格式
        
        Args:
            openai_content: OpenAI格式的内容列表
            
        Returns:
            List[Dict[str, Any]]: Responses API格式的内容列表
        """
        responses_content = []
    ...
    def convert_to_openai_format(self, responses_content: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """从Responses API格式转换为OpenAI Chat Completions格式
        
        Args:
            responses_content: Responses API格式的内容列表
            
        Returns:
            List[Dict[str, Any]]: OpenAI格式的内容列表
        """
        openai_content = []
