...
class EnvironmentChecker(IEnvironmentChecker):
    ...
    def check_environment_variables(self, required_vars: List[str]) -> List[CheckResult]:
        """检查环境变量"""
        results = []
        ...
        for var in required_vars:
            ...
            if value is not None:
                results.append(CheckResult(
                    ...
                    details={"variable": var, "value": "***" if "key" in var.lower() or "secret" in var.lower() else value}
                ))
            else:
                results.append(CheckResult(
                    component=f"env_var_{var}",
