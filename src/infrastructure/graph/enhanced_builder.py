...
class EnhancedGraphBuilder(GraphBuilder):
            ...
            try:
                ...
                if node_class:
                    ...
                    return self._wrap_node_function(
                        node_instance.execute, state_manager
                    )
            except ValueError:
                # 节点类型不存在，继续尝试其他方法
                pass
    ...
    def _wrap_node_function(
        ...
        if (
            ...
            and isinstance(getattr(function, "__self__", None), BaseNode)
        ):
            node_instance = getattr(function, "__self__")
            adapter_wrapper = EnhancedNodeWithAdapterExecutor(
                node_instance, state_manager
            )
        ...
        def wrapped_function(state: Union[WorkflowState, Dict[str, Any]]) -> Any:
            ...
            try:
                ...
                def node_executor(domain_state: Any) -> Any:
                    ...
                    # 将域状态转换为图状态
                    temp_graph_state = (
                        collaboration_adapter.state_adapter.to_graph_state(domain_state)
                    )
                    # 执行原始函数
                ...
                # 使用协作适配器执行
                return collaboration_adapter.execute_with_collaboration(
                    state, node_executor
                )
