...
class ExecutionStatsPlugin(IEndPlugin):
    ...
    def _collect_execution_stats(self, state: Dict[str, Any], context: PluginContext) -> Dict[str, Any]:
        """收集执行统计
    ...
    def _collect_execution_time_stats(self, state: Dict[str, Any], context: PluginContext) -> Dict[str, Any]:
        """收集执行时间统计
        ...
        try:
            ...
            if context.execution_start_time:
                ...
                time_stats["total_execution_time_formatted"] = self._format_duration(total_time)
            ...
            if "plugin_executions" in state:
                ...
                if plugin_times:
                    ...
                    fastest = min(plugin_executions, key=lambda p: p.get("execution_time", 0))
    ...
    def _get_memory_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_stats["process"] = {
                ...
                "percent": round(process_memory.percent, 2) if hasattr(process_memory, 'percent') else None
            }
            
            # 内存详细信息
            if self._config["include_memory_profiling"]:
                memory_details = process.memory_full_info() if hasattr(process, 'memory_full_info') else None
                if memory_details:
                    memory_stats["process"]["details"] = {
                        "uss_mb": round(memory_details.uss / (1024 * 1024), 2),
                        "pss_mb": round(memory_details.pss / (1024 * 1024), 2),
                        "shared_mb": round(memory_details.shared / (1024 * 1024), 2)
                    }
            ...
            if "node_executions" in state:
                ...
                for node_type, executions in node_types.items():
                    ...
                    node_stats[node_type] = {
                        ...
                        "average_time": round(sum(times) / len(times), 3) if times else 0,
    ...
    def _collect_plugin_performance_stats(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """收集插件性能统计
        ...
        try:
            if "plugin_executions" in state:
                ...
                # 成功率统计
                successful = len([e for e in plugin_executions if e.get("status") == "success"])
                plugin_stats["success_rate"] = round(successful / len(plugin_executions) * 100, 2) if plugin_executions else 0
    ...
    def _generate_stats_report(self, stats: Dict[str, Any], context: PluginContext) -> str:
        """生成统计报告
        ...
        if "execution_time" in stats:
            ...
            if "total_execution_time" in time_stats:
                lines.append(f"- **总执行时间**: {time_stats['total_execution_time_formatted']}")
            
            if "plugin_execution" in time_stats:
                plugin_exec = time_stats["plugin_execution"]
                lines.append(f"- **插件执行时间**: {plugin_exec['total_time']}s (平均: {plugin_exec['average_time']}s)")
                lines.append(f"  - 最慢插件: {plugin_exec['slowest']['name']} ({plugin_exec['slowest']['time']}s)")
                lines.append(f"  - 最快插件: {plugin_exec['fastest']['name']} ({plugin_exec['fastest']['time']}s)")
        ...
        if "resource_usage" in stats:
            ...
            if "memory" in resource_stats:
                ...
                if "system" in memory:
                    ...
                    lines.append(f"- **系统内存**: {sys_mem['used_mb']}MB / {sys_mem['total_mb']}MB ({sys_mem['percent']}%)")
            ...
            if "cpu" in resource_stats:
                ...
                if "system" in cpu:
                    sys_cpu = cpu["system"]
                    lines.append(f"- **系统CPU**: {sys_cpu['percent']}% ({sys_cpu['count']} 核心)")
        ...
        if "plugin_performance" in stats:
            ...
            for plugin_type, stats_data in plugin_perf.items():
                if isinstance(stats_data, dict) and "count" in stats_data:
                    lines.append(f"- **{plugin_type}**: {stats_data['count']} 次，平均 {stats_data['average_time']}s")
    ...
    def _save_stats_to_file(self, stats: Dict[str, Any], context: PluginContext) -> None:
        """保存统计到文件
