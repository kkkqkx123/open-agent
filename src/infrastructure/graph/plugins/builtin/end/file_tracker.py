...
class FileTrackerPlugin(IEndPlugin):
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        self._config = {
            ...
            "exclude_patterns": config.get("exclude_patterns", ["*.tmp", "*.log", "__pycache__", ".git"]),
    ...
    def execute(self, state: Dict[str, Any], context: PluginContext) -> Dict[str, Any]:
        ...
        try:
            ...
            # 比较快照，检测变更
            changes = self._compare_snapshots(self._initial_snapshot, current_snapshot)
    ...
    def _compare_snapshots(self, initial: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
        """比较文件快照，检测变更
            ...
            for file_path, file_info in current_files.items():
                if file_path not in initial_files:
                    ...
                    if self._config["track_created_files"]:
                        ...
                        changes["all"].append({"type": "created", "path": file_path, "info": file_info})
                else:
                    # 检查是否修改
            ...
            for file_path, file_info in initial_files.items():
                if file_path not in current_files:
                    if self._config["track_deleted_files"]:
                        ...
                        changes["all"].append({"type": "deleted", "path": file_path, "info": file_info})
    ...
    def _is_file_modified(self, initial_info: Dict[str, Any], current_info: Dict[str, Any]) -> bool:
        """检查文件是否修改
    ...
    def _analyze_changes(self, changes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_info in changes.get("created", []):
                ...
                all_files_with_size.append({"path": file_info.get("path"), "size": size, "type": "created"})
            ...
            for change in changes.get("modified", []):
                ...
                all_files_with_size.append({"path": change.get("path"), "size": size, "type": "modified"})
    ...
    def _generate_diff_report(self, changes: Dict[str, Any], analysis: Dict[str, Any]) -> str:
        """生成差异报告
        ...
        if analysis.get("by_extension"):
            ...
            for ext, stats in analysis["by_extension"].items():
                lines.append(f"- **{ext}**: 创建 {stats['created']}, 修改 {stats['modified']}, 删除 {stats['deleted']}")
            lines.append("")
        ...
        if analysis.get("by_directory"):
            ...
            for directory, stats in analysis["by_directory"].items():
                lines.append(f"- **{directory}**: 创建 {stats['created']}, 修改 {stats['modified']}, 删除 {stats['deleted']}")
            lines.append("")
        ...
        if analysis.get("size_analysis"):
            ...
            lines.append(f"- **创建文件总大小**: {self._format_size(size_analysis.get('total_size_created', 0))}")
            lines.append(f"- **修改文件总大小**: {self._format_size(size_analysis.get('total_size_modified', 0))}")
            lines.append(f"- **删除文件总大小**: {self._format_size(size_analysis.get('total_size_deleted', 0))}")
            
            if size_analysis.get("largest_file"):
                largest = size_analysis["largest_file"]
                lines.append(f"- **最大文件**: {largest['path']} ({self._format_size(largest['size'])})")
            
            if size_analysis.get("smallest_file"):
                smallest = size_analysis["smallest_file"]
                lines.append(f"- **最小文件**: {smallest['path']} ({self._format_size(smallest['size'])})")
    ...
    def _save_file_tracking_report(self, changes: Dict[str, Any], analysis: Dict[str, Any], context: PluginContext) -> None:
        """保存文件追踪报告
        ...
        try:
            ...
            if "diff_report" in analysis:
                md_filename = f"file_tracking_report_{context.workflow_id}_{timestamp}.md"
