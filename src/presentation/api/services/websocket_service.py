...
class WebSocketService:
    ...
    async def _handle_stream_update(self, data: dict, session_id: Optional[str]):
        """处理流式更新"""
        if session_id:
...
async def push_performance_metrics(metrics_data: dict, session_id: Optional[str] = None):
    """推送性能指标"""
    await websocket_service.broadcast_event("performance_metrics", metrics_data, session_id)


async def push_error_event(error_data: dict, session_id: Optional[str] = None):
    """推送错误事件"""
