...
class ConfigValidator(IConfigValidator):
    ...
    def validate_llm_config(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if result.is_valid:
            # 检查API密钥是否已配置（如果需要）
            if config.get('model_type') in ['openai', 'gemini', 'anthropic'] and not config.get('api_key'):
    ...
    def validate_config_structure(self, config: Dict[str, Any], required_fields: List[str]) -> ValidationResult:
        """验证配置结构
    ...
    def validate_config_types(self, config: Dict[str, Any], type_mapping: Dict[str, type]) -> ValidationResult:
        """验证配置类型
    ...
    def validate_config_values(self, config: Dict[str, Any], value_constraints: Dict[str, Any]) -> ValidationResult:
        """验证配置值
        ...
        for field, constraints in value_constraints.items():
            if field in config:
                ...
                if 'enum' in constraints and value not in constraints['enum']:
                    result.add_error(f"字段 '{field}' 值无效，允许的值: {constraints['enum']}")
                
                # 检查范围
                if 'min' in constraints and value < constraints['min']:
                    result.add_error(f"字段 '{field}' 值过小，最小值: {constraints['min']}")
                
                if 'max' in constraints and value > constraints['max']:
                    result.add_error(f"字段 '{field}' 值过大，最大值: {constraints['max']}")
                
                # 检查长度
                if hasattr(value, '__len__'):
                    if 'min_length' in constraints and len(value) < constraints['min_length']:
                        result.add_error(f"字段 '{field}' 长度过短，最小长度: {constraints['min_length']}")
                    
                    if 'max_length' in constraints and len(value) > constraints['max_length']:
                        result.add_error(f"字段 '{field}' 长度过长，最大长度: {constraints['max_length']}")
                
                # 检查正则表达式
                if 'pattern' in constraints and not constraints['pattern'].match(str(value)):
