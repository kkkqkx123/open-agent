...
class IHookPlugin(IPlugin):
    ...
    def metadata(self) -> PluginMetadata:
        """获取插件元数据"""
        meta = super().metadata
        if meta.plugin_type != PluginType.HOOK:
            # 确保插件类型正确
            meta.plugin_type = PluginType.HOOK
        return meta
...
class IStartPlugin(IPlugin):
    ...
    def metadata(self) -> PluginMetadata:
        """获取插件元数据"""
        meta = super().metadata
        if meta.plugin_type != PluginType.START:
            # 确保插件类型正确
            meta.plugin_type = PluginType.START
        return meta
...
class IEndPlugin(IPlugin):
    ...
    def metadata(self) -> PluginMetadata:
        """获取插件元数据"""
        meta = super().metadata
        if meta.plugin_type != PluginType.END:
            # 确保插件类型正确
            meta.plugin_type = PluginType.END
        return meta
