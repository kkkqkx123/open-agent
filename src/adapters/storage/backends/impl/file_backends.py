...
class FileSessionBackend(BaseStorageBackend, ISessionStorage):
    """文件会话存储后端
    
    通过组合文件提供者和会话存储混入来实现。
    """
    
    def __init__(self, base_path: str = "./sessions", **config: Any) -> None:
        """初始化文件会话后端
    ...
    async def _connect_impl(self) -> None:
        ...
        logger.debug("File session backend connected and initialized")
    
    async def _disconnect_impl(self) -> None:
        """实际断开连接实现"""
        await self._provider.disconnect()
        logger.debug("File session backend disconnected")
...
class FileThreadBackend(BaseStorageBackend, IThreadStorage):
    """文件线程存储后端
    
    通过组合文件提供者和线程存储混入来实现。
    """
    
    def __init__(self, base_path: str = "./threads", **config: Any) -> None:
        """初始化文件线程后端
    ...
    async def _connect_impl(self) -> None:
        ...
        logger.debug("File thread backend connected and initialized")
    
    async def _disconnect_impl(self) -> None:
        """实际断开连接实现"""
        await self._provider.disconnect()
        logger.debug("File thread backend disconnected")
