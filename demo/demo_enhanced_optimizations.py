...
展示第一阶段实现的EnhancedStateManager和EnhancedConfigValidator功能。
"""
...
def demo_enhanced_config_validator():
    ...
    test_configs = [
        ...
        },
        {
            ...
            "nodes": {"node1": {"type": "input"}},
            "performance": {
                "cache": {"enabled": True, "max_size": 50000},  # 过大缓存
                "concurrency": {"max_workers": 200}  # 过大并发数
            }
        }
    ]
...
# 缺少version和nodes字段
    ...
    try:
        fixed_config = fixer.auto_fix_config(temp_file)
        print("修复后的配置:")
...
def demo_integration():
    ...
    # 模拟多个线程同时修改状态
    initial_state = create_agent_state("初始输入")
    initial_state["step_results"] = ["步骤1完成"]
    
    # 线程1的修改
    thread1_state = initial_state.copy()
    thread1_state["input"] = "线程1修改的输入"
    thread1_state["step_results"].append("步骤2完成")
    
    # 线程2的修改
    thread2_state = initial_state.copy()
    thread2_state["input"] = "线程2修改的输入"
    thread2_state["custom_data"] = {"key": "value"}
    
    print("初始状态:", {k: v for k, v in initial_state.items() if v})
    print("线程1状态:", {k: v for k, v in thread1_state.items() if v})
    print("线程2状态:", {k: v for k, v in thread2_state.items() if v})
    
    # 解决冲突
    resolved_state, unresolved = state_manager.update_state_with_conflict_resolution(
        thread1_state, thread2_state
    )
