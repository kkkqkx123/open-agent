...
""",
            ...
            # 3. 创建有效配置并验证恢复
            container.create_test_config(
                "configs/valid.yaml", {"log_level": "INFO", "env": "test"}
            )
            ...
            # 应该检测到违规
            layer_violations = [
                r for r in arch_results if r.component == "architecture_layer"
            ]
    ...
    def test_multi_environment_workflow(self) -> None:
        ...
        with TestContainer() as container:
            ...
            container.create_test_config(
                "configs/global.yaml",
                {"log_level": "INFO", "env": "development", "debug": True},
            )

            container.create_test_config(
                "configs/production.yaml",
                {"log_level": "WARNING", "env": "production", "debug": False},
            )
            ...
            di_container.register_factory(
                ConfigService, lambda: ConfigService("global.yaml"), "development"
            )
            di_container.register_factory(
                ConfigService, lambda: ConfigService("production.yaml"), "production"
            )
