...
from src.infrastructure.exceptions import ServiceNotRegisteredError, ConfigurationError
...
"""
    ...
    def test_architecture_checker_integration(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 检查层级违规
            layer_result = next(
                r for r in results if r.component == "architecture_layer"
            )
            assert layer_result.status in ["PASS", "ERROR"]

            # 检查循环依赖
            circular_result = next(
                r for r in results if r.component == "circular_dependency"
            )
