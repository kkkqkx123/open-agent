...
from src.config.config_validator import ConfigValidator, ValidationResult
from src.config.models.global_config import GlobalConfig
from src.config.models.llm_config import LLMConfig
from src.config.models.agent_config import AgentConfig
from src.config.models.tool_config import ToolConfig
...
class TestConfigValidator:
    ...
    def test_validate_global_config_production_with_debug(self):
        ...
        assert result.is_valid  # 基本验证通过
        assert result.has_warnings()  # 但有警告
        assert any("生产环境" in warning and "调试模式" in warning for warning in result.warnings)
    ...
    def test_validate_global_config_file_log_no_path(self):
        ...
        assert result.is_valid  # 基本验证通过
        assert result.has_warnings()  # 但有警告
        assert any("文件日志" in warning and "路径" in warning for warning in result.warnings)
