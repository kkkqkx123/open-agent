...
class TestLogRedactor:
    ...
    def test_default_patterns(self):
        ...
        # 测试API Key脱敏
        assert (
            redactor.redact("API Key: sk-abc123def4567890123456") == "API Key: sk-***"
        )
        ...
        # 测试Token脱敏
        assert (
            redactor.redact('{"token": "abc123def4567890123456789"}')
            == '{"token": "***TOKEN***"}'
        )

        # 测试API Key字段脱敏
        assert (
            redactor.redact('{"api_key": "secret123456789012"}')
            == '{"api_key": "***APIKEY***"}'
        )

        # 测试Bearer Token脱敏
        assert (
            redactor.redact("Authorization: Bearer abc123def4567890123456")
...
class TestCustomLogRedactor:
    ...
    def test_init_with_config(self):
        ...
        config = {
            "hash_sensitive": True,
            "patterns": [
                {"pattern": r"custom_secret_\d+", "replacement": "CUSTOM_SECRET"}
            ],
        }
    ...
    def test_update_config(self):
        ...
        new_config = {
            "hash_sensitive": True,
            "patterns": [
                {"pattern": r"new_pattern_\w+", "replacement": "NEW_REPLACEMENT"}
            ],
        }
