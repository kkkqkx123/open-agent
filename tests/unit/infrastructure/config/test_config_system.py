...
class TestConfigSystem:
    ...
    def test_load_config_with_inheritance(self):
        ...
        assert (
            config_data["headers"]["User-Agent"] == "ModularAgent/1.0"
        )  # 继承自组配置
    ...
    def test_load_config_with_inheritance_group_not_found(self):
        ...
        config_data = self.config_system._load_config_with_inheritance(
            "llms", "invalid"
        )
    ...
    def test_load_config_validation_error(self):
        """测试加载配置验证错误"""
        # 创建无效的LLM配置文件
        llm_config = {"model_type": "invalid_type", "model_name": "gpt-4"}  # 无效类型
