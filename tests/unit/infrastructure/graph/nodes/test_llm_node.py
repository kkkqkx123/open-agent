...
class TestLLMNode:
    ...
    @pytest.fixture
    def node(self, mock_llm_client: Mock) -> LLMNode:
        """创建LLM节点实例"""
    ...
    def sample_state(self) -> AgentState:
        """示例状态"""
        # 使用domain层的AgentState（数据类）
        state = AgentState(
            agent_id="test-agent",
            agent_type="test",
            messages=[
                AgentMessage(content="用户输入", role="human"),
                AgentMessage(content="分析结果", role="ai")
            ],
            tool_results=[ToolResult(success=True, output="工具执行结果", tool_name="test_tool")],
            max_iterations=10
        )
    ...
    @pytest.fixture
    def sample_llm_response(self) -> LLMResponse:
        """示例LLM响应"""
        return LLMResponse(
            content="LLM生成的响应",
    ...
    def test_init(self, mock_llm_client: Mock) -> None:
        """测试初始化"""
    ...
    def test_node_type_property(self, node: LLMNode) -> None:
        """测试节点类型属性"""
        assert node.node_type == "llm_node"

    def test_execute_success(self, node: LLMNode, mock_llm_client: Mock, sample_state: AgentState, sample_config: Dict[str, Any], sample_llm_response: LLMResponse) -> None:
        """测试执行成功"""
        # 配置模拟
    ...
    def test_execute_with_next_node(self, node: LLMNode, mock_llm_client: Mock, sample_state: AgentState, sample_config: Dict[str, Any], sample_llm_response: LLMResponse) -> None:
        """测试执行并指定下一个节点"""
        # 配置模拟
        mock_llm_client.generate.return_value = sample_llm_response
        mock_llm_client.get_model_info.return_value = {"model": "test-model"}
    ...
    def test_execute_with_follow_up(self, node: LLMNode, mock_llm_client: Mock, sample_state: AgentState, sample_config: Dict[str, Any]) -> None:
        """测试执行并需要进一步处理"""
        # 创建需要进一步处理的响应
    ...
    def test_get_config_schema(self, node: LLMNode) -> None:
        """测试获取配置模式"""
    ...
    def test_get_llm_client_with_instance(self, mock_llm_client: Mock) -> None:
        """测试获取LLM客户端（使用实例）"""
        node = LLMNode(mock_llm_client)
        result = node._get_llm_client({})
        assert result == mock_llm_client

    def test_get_llm_client_without_instance(self) -> None:
        """测试获取LLM客户端（不使用实例）"""
        node = LLMNode()
        result = node._get_llm_client({})
    ...
    def test_build_system_prompt_with_template(self, node: LLMNode) -> None:
        """测试构建系统提示词（使用模板）"""
        # 使用domain层的AgentState（数据类）
        state = AgentState(
            agent_id="test-agent",
    ...
    def test_build_system_prompt_with_tool_results(self, node: LLMNode, sample_state: AgentState) -> None:
        """测试构建系统提示词（包含工具结果）"""
        config = {
            "system_prompt": "基础提示词",
            "include_tool_results": True
    ...
    def test_process_prompt_template(self, node: LLMNode) -> None:
        """测试处理提示词模板"""
        ...
        state = AgentState(
            ...
            agent_type="test",
            messages=[
                AgentMessage(content="消息1", role="human"),
                AgentMessage(content="消息2", role="ai")
            ],
            tool_results=[],
            max_iterations=10
        )
    ...
    def test_get_default_system_prompt(self, node: LLMNode) -> None:
        """测试获取默认系统提示词"""
    ...
    def test_format_tool_results_empty(self, node: LLMNode) -> None:
        """测试格式化空工具结果"""
        tool_results: List[Any] = []
        formatted = node._format_tool_results(tool_results)
        assert formatted == "没有工具执行结果"

    def test_prepare_messages(self, node: LLMNode, sample_state: AgentState) -> None:
        """测试准备消息"""
        system_prompt = "系统提示词"
    ...
    def test_truncate_messages_for_context(self, node: LLMNode) -> None:
        """测试截断消息以适应上下文"""
    ...
    def test_prepare_parameters(self, node: LLMNode) -> None:
        """测试准备参数"""
        config = {
            "max_tokens": 1000,
            "temperature": 0.7,
    ...
    def test_determine_next_node_with_config(self, node: LLMNode, sample_config: Dict[str, Any]) -> None:
        """测试确定下一个节点（配置指定）"""
        sample_config["next_node"] = "next_node"
        response = Mock()
    ...
    def test_determine_next_node_with_follow_up(self, node: LLMNode, sample_config: Dict[str, Any]) -> None:
        """测试确定下一个节点（需要进一步处理）"""
    ...
    def test_determine_next_node_default(self, node: LLMNode, sample_config: Dict[str, Any]) -> None:
        """测试确定下一个节点（默认情况）"""
        sample_config.pop("next_node", None)  # 确保没有配置下一个节点
        response = Mock()
        response.content = "普通响应"
    ...
    def test_needs_follow_up_true(self, node: LLMNode) -> None:
        """测试需要进一步处理（真）"""
        content = "我需要更多信息来回答这个问题"
        result = node._needs_follow_up(content)
        assert result is True

    def test_needs_follow_up_false(self, node: LLMNode) -> None:
        """测试需要进一步处理（假）"""
        content = "这是完整的答案"
        result = node._needs_follow_up(content)
