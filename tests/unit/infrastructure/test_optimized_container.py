...
class TestOptimizedDependencyContainer:
    ...
    def test_service_with_multiple_dependencies(self):
        ...
        assert isinstance(service.dep2, TestService)
    ...
    def test_service_creation_path_cache(self):
        ...
        # 获取服务创建路径
        path = self.container.get_service_creation_path(TestService)
        
        # 验证路径包含依赖
        assert DependencyService in path
        assert TestService in path
    ...
    def test_path_cache_functionality(self):
        ...
        # 获取创建路径（应该使用缓存）
        path1 = self.container.get_service_creation_path(TestService)
        path2 = self.container.get_service_creation_path(TestService)
        
        # 路径应该相同
        assert path1 == path2
        assert len(path1) >= 2  # 至少包含TestService和DependencyService
