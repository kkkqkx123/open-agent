...
class TestLLMMetric:
    ...
    def test_llm_metric_to_dict(self) -> None:
        """测试LLM指标转换为字典"""
        metric = LLMMetric(
            model="gpt-4", input_tokens=100, output_tokens=50, total_time=2.5
        )
...
class TestToolMetric:
    ...
    def test_tool_metric_creation(self) -> None:
        """测试工具指标创建"""
        metric = ToolMetric(
            tool="search_tool", count=10, success_count=8, total_time=5.0
        )
...
class TestSessionMetric:
    ...
    def test_total_llm_calls(self) -> None:
        ...
        session.llm_calls.append(
            LLMMetric("gpt-4", input_tokens=10, output_tokens=50, total_time=2.0)
        )
        session.llm_calls.append(
            LLMMetric(
                "gpt-3.5", input_tokens=50, output_tokens=25, total_time=1.0, count=2
            )
        )
    ...
    def test_total_tokens(self) -> None:
        ...
        session.llm_calls.append(
            LLMMetric("gpt-3.5", input_tokens=50, output_tokens=25, total_time=1.0)
        )
    ...
    def test_session_to_dict(self) -> None:
        ...
        session.tool_calls.append(
            ToolMetric("search", count=5, success_count=4, total_time=2.0)
        )
        session.total_messages += 1
        session.total_errors += 1
