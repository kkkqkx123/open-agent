...
class TestAgentStateManager:
    ...
    def test_create_initial_state(self):
        ...
        assert initial_state.agent_config == config
        assert initial_state.workflow_name == "test_workflow"
        assert initial_state.iteration_count == 0
        assert initial_state.max_iterations == 10  # 默认值
        assert len(initial_state.memory) == 0
    ...
    def test_update_state_with_memory(self):
        ...
        assert len(updated_state.memory) == 2
        assert updated_state.memory[0].content == "Hello"
        assert updated_state.memory[1].content == "Hi there!"
    ...
    def test_reset_state_for_new_task(self):
        ...
        assert len(reset_state.memory) == 0
