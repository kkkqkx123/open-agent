...
class TestBaseAgent:
    ...
    async def test_execute_with_success(self):
        ...
        # 模拟执行逻辑方法
        self.base_agent._execute_logic = AsyncMock(return_value=input_state)
    ...
    async def test_execute_with_error(self):
        ...
        # 不需要模拟_execute_logic，因为ConcreteAgent的_execute_logic会直接返回状态
        # 我们需要在状态验证阶段抛出异常
        self.base_agent.validate_state = Mock(return_value=False)
        
        # 执行并捕获异常
        with pytest.raises(ValueError, match="状态验证失败，Agent test_agent 无法处理当前状态"):
    ...
    async def test_execute_with_retry_mechanism(self):
        ...
        # 执行（应该成功，因为有重试机制）
        result_state = await retry_agent.execute_with_retry(input_state, {}, max_retries=3)
        
        # 验证重试了3次（2次失败 + 1次成功）
        assert retry_agent.call_count == 3
        assert retry_agent.execution_stats["total_executions"] == 3  # 每次尝试都会增加计数
    ...
    async def test_execute_tool_async(self):
        ...
        # 验证工具执行器被调用
        self.mock_tool_executor.execute_async.assert_called_once_with(
